// The purpose of this script is to send temperature data to two receiver devices.
// The first device is a LILYGO T-Display S3
// The second defive is a XIAO ESP32C3 connected to a Seeed Studio Round Display.
...
// LILYGO T-Display S3
uint8_t receiverAdd1[] = {0x3C, 0x84, 0x27, 0xC1, 0x39, 0x54};
// XIAO ESP32C3 on Seeed Studio Round Display
uint8_t receiverAdd2[] = {0x64, 0xe8, 0x33, 0x84, 0x79, 0xbc};
//64:e8:33:84:79:bc
...
void setup() {
  ...
  // Register first peer
  memcpy(peerInfo.peer_addr, receiverAdd1, 6);
  if (esp_now_add_peer(&peerInfo) != ESP_OK){
    Serial.println("Failed to add peer.");
  }

  // Register second peer
  memcpy(peerInfo.peer_addr, receiverAdd2, 6);
  if (esp_now_add_peer(&peerInfo) != ESP_OK){
    Serial.println("Failed to add peer.");
  }
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    read();
    esp_now_send(0, (uint8_t *) &myData, sizeof(myData));  //0 as first arg sends to all peers
  }
}
